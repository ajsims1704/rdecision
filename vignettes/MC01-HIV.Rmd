...
s.mono.A <- MarkovState$new("A: 200 < cd4 < 500", cost=dmca+ccca+cAZT)
s.mono.B <- MarkovState$new("B: cd4 < 200", cost=dmcb+cccb+cAZT)
s.mono.C <- MarkovState$new("C: AIDS", cost=dmcc+cccc+cAZT)
s.mono.D <- MarkovState$new("D: Death", cost=0)
# create transitions
...
f1c  <- "Markov model for monotherapy"
```
...
done for the first 20 cycles of the model. In addition, the proportion
of patients alive at each cycle (the Markov trace) is added to the table.  
...
```{r print_monocycle}
pander::pander(DF.mono, row.names=FALSE, justify="rrrrrrr", round=0)
```
