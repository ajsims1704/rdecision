...
(class `MarkovTransition`) may have various properties whose values reflect the
...
cycle is equivalent to the survival function. Transition rates will be set
later from probabilities.
```{r model, echo=TRUE}
...
tABm <- MarkovTransition$new(sAm, sBm)
tACm <- MarkovTransition$new(sAm, sCm)
tADm <- MarkovTransition$new(sAm, sDm)
tBBm <- MarkovTransition$new(sBm, sBm)
tBCm <- MarkovTransition$new(sBm, sCm)
tBDm <- MarkovTransition$new(sBm, sDm)
tCCm <- MarkovTransition$new(sCm, sCm)
tCDm <- MarkovTransition$new(sCm, sDm)
...
in 1 year as 
transition probabilities by dividing counts by the total transitions observed
from each state. With this assumption, the annual (per-cycle)
transition probabilities are calculated as follows. Via the `set_rates` 
function, transition rates for the model can be set from the per cycle 
transition probabilities.
```{r setrates, echo=TRUE}
nA <- nAA + nAB + nAC + nAD
nB <- nBB + nBC + nBD
nC <- nCC + nCD
Pt <- matrix(
...
end is directly related to the per-cycle probability. Following the work of
Miller and Homan [-@miller1994], Welton and Ades [-@welton2005], Jones *et al*
[-@jones2017] and others, `rdecision` prefers rates to probabilities as input 
to models. These can be specified when transitions are created, or by using
the `set_rate` method of `MarkovTransition`. Per-cycle transition probabilities
can be extracted for checking. The alternative approach of specifying per-cycle
probabilities is supported via the `set_rates` method of `CohortMarkovModel` 
(as in this example). Conversion between rates and probabilities for 
multi-state Markov models is non-trivial [@jones2017] and it is recommended 
to specify either rates or probabilities and not to mix them during input.
...
tABc <- MarkovTransition$new(sAc, sBc)
tACc <- MarkovTransition$new(sAc, sCc)
tADc <- MarkovTransition$new(sAc, sDc)
tBBc <- MarkovTransition$new(sBc, sBc)
tBCc <- MarkovTransition$new(sBc, sCc)
tBDc <- MarkovTransition$new(sBc, sDc)
tCCc <- MarkovTransition$new(sCc, sCc)
tCDc <- MarkovTransition$new(sCc, sDc)
...
# set the rates by specifying probabilities
m.comb$set_rates(Ptc, tcycle)
```
