...
these are converted to instantaneous hazard rates in units of events/year. Note
that this is done by calculating the total probability of leaving each state,
converting it to a total rate, then partitioning it into rates for each
of the transitions leaving the state. 
...
was applied to the probabilities, and the same process is applied to convert
probabilities into transition rates.
```{r combo, echo=TRUE}
# annual probabilities
pAB <- 0.202*RR 
pAC <- 0.067*RR
