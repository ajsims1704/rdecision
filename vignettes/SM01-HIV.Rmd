...
#| purl = FALSE
gbp <- function(x) {
  s <- format(round(x, digits = 0L), digits = NULL, nsmall = 0L,
...
# system) and so DiagrammeR only needs to be available there, not on any
...
try(
  DiagrammeR::grViz(m$as_DOT()), silent = TRUE
)
```
