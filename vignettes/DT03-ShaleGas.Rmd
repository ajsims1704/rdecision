...
local({
  ...
  stopifnot(
    ...
    all.equal(M[[as.character(it8), "Cost"]], 220.5, tolerance = 1.0)
  )
})
```
...
local({
  ...
  stopifnot(
    all.equal(sum(RES[itss, "Probability"]), 1.0, tolerance = 0.01),
    all.equal(sum(RES[itss, "Cost"]), 50.0, tolerance = 5.0),
    all.equal(sum(RES[itss, "Benefit"]), 888.0, tolerance = 5.0)
  )
})
```
