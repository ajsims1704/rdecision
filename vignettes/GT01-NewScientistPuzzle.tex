...
\NormalTok{A <-}\StringTok{ }\NormalTok{V[[}\DecValTok{1}\NormalTok{]]}
\NormalTok{B <-}\StringTok{ }\NormalTok{V[[}\DecValTok{25}\NormalTok{]]}
\NormalTok{P <-}\StringTok{ }\NormalTok{G}\OperatorTok{$}\KeywordTok{paths}\NormalTok{(A,B)}
\CommentTok{# convert paths to walks}
\NormalTok{W <-}\StringTok{ }\KeywordTok{lapply}\NormalTok{(P,}\ControlFlowTok{function}\NormalTok{(p)\{G}\OperatorTok{$}\KeywordTok{walk}\NormalTok{(p)\})}
\CommentTok{# count and tabulate how many special edges each walk traverses}
\NormalTok{BB <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"V11"}\NormalTok{, }\StringTok{"H22"}\NormalTok{, }\StringTok{"V25"}\NormalTok{, }\StringTok{"H33"}\NormalTok{, }\StringTok{"V32"}\NormalTok{, }\StringTok{"H44"}\NormalTok{, }\StringTok{"V43"}\NormalTok{)}
\NormalTok{nw <-}\StringTok{ }\KeywordTok{sapply}\NormalTok{(W, }\ControlFlowTok{function}\NormalTok{(w) \{}
\NormalTok{  lv <-}\StringTok{ }\KeywordTok{sapply}\NormalTok{(w, }\ControlFlowTok{function}\NormalTok{(e) \{e}\OperatorTok{$}\KeywordTok{label}\NormalTok{() }\OperatorTok{%in%}\StringTok{ }\NormalTok{BB\}) }
  \KeywordTok{return}\NormalTok{(}\KeywordTok{sum}\NormalTok{(lv))}
\NormalTok{\})}
\CommentTok{# tabulate }
\NormalTok{ct <-}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(}\KeywordTok{table}\NormalTok{(nw))}
\end{Highlighting}
\end{Shaded}
