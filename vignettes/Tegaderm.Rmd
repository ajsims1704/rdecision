...
Variables in the model may be included in the decision tree via expressions,
which are mathematical expressions which involve model variables and are themselves
...
`uncorrelate=TRUE` in method `evaluateChoices` of the decision node.
...
is `r round(sqrt(sd.T^2 + sd.S^2 - 2*cov.TS),2)`$^2$ and of the
uncorrelated model variables 
is `r round(sqrt(sd.T^2 + sd.S^2),2)`$^2$, close to the that observed.
