...
is the node representing the decision problem. The time horizon is not stated
explicitly in the model, and is assumed to be 7 days here. It was implied
...
state.S.Dermatitis <- State$new(
  "Dermatitis (Standard Dressing)", cost=c.Dermatitis.S, interval=th
)
state.S.LSI <- State$new(
  "Local site infection (Standard Dressing)", cost=c.LSI.S, interval=th
)
state.S.CRBSI <- State$new(
  "CRBSI (Standard Dressing)", cost=c.CRBSI.S, interval=th
)
state.S.NoComp <- State$new(
  "No complication (Standard Dressing)", cost=c.S, interval=th
)

chance.S <- ChanceNode$new(
  children = list(state.S.Dermatitis, state.S.LSI, state.S.CRBSI, state.S.NoComp),
...
state.T.Dermatitis <- State$new(
  "Dermatitis (Tegaderm CHG)", cost=c.Dermatitis.T, interval=th
)
state.T.LSI <- State$new(
  "Local site infection (Tegaderm CHG)", cost=c.LSI.T, interval=th
)
state.T.CRBSI <- State$new(
  "CRBSI (Tegaderm CHG)", cost=c.CRBSI.T, interval=th
)
state.T.NoComp <- State$new(
  "No complication (Tegaderm CHG)", cost=c.T, interval=th
)

chance.T <- ChanceNode$new(
  children = list(state.T.Dermatitis, state.T.LSI, state.T.CRBSI, state.T.NoComp),
  edgelabels = c('Dermatitis', 'Local site infection', 'CRBSI', 'No complication'),
  costs = list(0, 0, 0, 0),
...
local({
  DF <- d$tabulate_modvars(include.descendants=T, include.operands=T)
