...
is similar, but aggregates the results by choice.
...
yields the following result. There are, as expected, eight root-to-leaf pathways, and 
...
## Components of cost
Each component of the cost of the technology and its comparator is as follows:
```{r treefree, echo=T}
...
junk <- ExpressionModelVariable$new(
  'junk',
  'S-T', 'GBP',
  quote((CHG.S+CRBSI.S+LSI.S+Dermatitis.S)-(CHG.T+CRBSI.T+LSI.T+Dermatitis.T))
)

```

## Evaluating the decision
Although the costs can be analyzed without incorporating them into a tree, 
a `DecisionNode` object provides easy methods for describing the model
and evaluating decisions. A rudimentary tree can be constructed as follows:
```{r treefree-decision, echo=TRUE}
leaf.S <- LeafNode$new('Standard dressing')
leaf.T <- LeafNode$new('Tegaderm')
dd <- DecisionNode$new(
  children <- c(leaf.S, leaf.T),
  edgelabels = c('Standard-Tegaderm Dressing', 'Zero'),
  costs = list(junk, 0) #list(total.S, total.T) 
)
```

## Base case
The components of cost can be extracted from the ExpressionModelVariable `dd` by 
tabulating the model variables in the decision node. In this case, only the
rows containing component costs are displayed.
```{r treefree-base, echo=FALSE}
local({
  MV <- dd$tabulateModelVariables(include.operands=TRUE)
  vars <- c('CHG.S', 'CRBSI.S', 'LSI.S', 'Dermatitis.S',
            'CHG.T', 'CRBSI.T', 'LSI.T', 'Dermatitis.T',
            'junk')
  keep <- c('Description', 'Units', 'Mean', 'Q2.5', 'Q97.5', 'Qhat')
  knitr::kable(MV[MV$Label %in% vars,keep], row.names=F, 
               format.args=list(scientific=F), digits=3
               )
})
```

```{r treefree-base-result, echo=FALSE}
SUM <- dd$evaluateChoices(expected=T)
PE.saving <<- SUM$Cost[SUM$Choice=='Standard-Tegaderm Dressing'] - 
              SUM$Cost[SUM$Choice=='Zero']
rm(SUM)
```

As with the full tree, the decision can be evaluated by calling
the `evaluateChoices` method. The point estimate of the saving is 
`r round(-1*PE.saving,2)`, which is identical to
the point estimate saving calculated using the decision tree approach.

## Probabilistic sensitivity analysis
```{r treefree-PSA, echo=FALSE}
local({
  N <- 1000
  RES <<- data.frame(
    Run = 1:N,
...
This proceeds exactly as with the full tree analysis. From PSA, the mean cost of
treatment with Tegaderm was `r round(mean(RES$Tegaderm),2)`,
the mean cost of treatment with standard dressings was `r round(mean(RES$Standard),2)`
and the mean cost saving was `r round(mean(RES$Difference),2)`. The 95% confidence 
interval for cost saving was `r round(quantile(RES$Difference, probs=c(0.025)),2)` to 
`r round(quantile(RES$Difference, probs=c(0.975)),2)`. 
