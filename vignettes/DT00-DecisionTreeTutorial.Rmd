...
roots <- c(file.path("."), file.path(".."), file.path("..", ".."))
descfile <- file.path(roots, "DESCRIPTION")
level <- file.exists(descfile)
stopifnot(sum(level) == 1L)
root <- roots[[which(level)]]
root <- normalizePath(root)
#rm(roots, descfile, level)
# read vignette source chunks from corresponding testthat script
rfile <- file.path(root, "tests", "testthat", "test-model-Lifestyle.R")
stopifnot(file.exists(rfile))
knitr::read_chunk(path = rfile)
```
...
```{r}
#| label = actions-probabilistic,
#| echo = TRUE
```

```{r}
#| label = reactions-probabilistic,
#| echo = TRUE
```
