...
# This .Rmd file is processed by knitr at build time (on the package developer's
# system) and so the dot tool only needs to be available there, not on any
# system used for processing the .md, including CRAN. 
figdir <- system.file("man", "figures", package = "rdecision")
pngfile <- file.path("man", "figures", "README-sb.png") 
dotfile <- file.path("man", "figures", "README-sb.gv")
# update the .gv file
DOT <- M$as_DOT()
writeLines(DOT, con = dotfile)
# if graphviz is installed, create the .png file from the .gv file with the dot
# command line tool, otherwise fail gracefully
pdot <- Sys.which("dot")
...
pander::pander(
  MT[rows, keep], row.names = FALSE, justify = "rrrrr", 
