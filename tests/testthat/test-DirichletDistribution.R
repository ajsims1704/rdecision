test_that("illegal initializations are rejected", {
  K <- as.integer(2)
  alpha <- c(2,6)
  expect_silent(DirichletDistribution$new(K, alpha))
  expect_error(DirichletDistribution$new("9",alpha), class="invalid_order")
  expect_error(DirichletDistribution$new(K,"0.5"), class="alpha_not_numeric")
  expect_error(DirichletDistribution$new(K,c(2,NA)), class="alpha_not_defined")
  expect_error(DirichletDistribution$new(K,c(2,0)), class="alpha_unsupported")
  expect_error(
    DirichletDistribution$new(K,c(2,1,1)), class="inconsistent_K_alpha"
  )
  expect_error(
    DirichletDistribution$new(as.integer(1), c(2)),
    class = "K_unsupported"
  )
})
...
test_that("the correct distribution name is created", {
  K <- as.integer(2)
  alpha <- c(2,6)
  D <- DirichletDistribution$new(K, alpha)
