...
test_that("child nodes are accepted and rejected correctly", {
  ...
  p <- c(0.5, 0.5)
  costs <- c(0, 0)
  expect_error(ChanceNode$new(c(s1), labels, p, costs), 
               class="incorrect_child_count")
  expect_error(ChanceNode$new(c(s1, TRUE), labels, p, costs), 
               class="non-Node_child")
  c <- ChanceNode$new(c(s1,s2), labels, costs, p)
...
test_that("edge labels are accepted and rejected correctly", {
  ...
  p <- c(0.5, 0.5)
  costs <- c(0, 0)
  expect_error(ChanceNode$new(c(s1, s2), c("to 1"), costs, p),
               class="incorrect_edge_label_count")
  expect_error(ChanceNode$new(c(s1, s2), list("to 1", 42), costs, p),
                class="non-string_edge_label")
  expect_silent(ChanceNode$new(c(s1, s2), c("to 1", 42), costs, p))
  c <- ChanceNode$new(c(s1, s2), labels, costs, p)
...
test_that("probabilities are accepted and rejected correctly", {
  ...
  p <- c(0.5, 0.5)
  costs <- c(0, 0)
  expect_error(ChanceNode$new(c(s1,s2), labels, costs, p, ptype=TRUE),
